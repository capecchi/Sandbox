...
if os.path.isdir('C:/Users/Owner'):
    ...
    dir = 'C:/Users/wcapecch/PycharmProjects/Sandbox/data/'
else:
...
for i, (lbl, file) in enumerate([('superior50k2018', k50), ('stonemill50M2020', m50)]):
    ...
    if shadow:  # linethick
        ...
        plt.plot(x, y1 + offset, '#6b8e23', path_effects=[pe.SimpleLineShadow(offset=(0, -2)), pe.Normal()])
        plt.plot([nn[1], nn[1]], [np.interp(nn[1], x, y) + offset - cross_height,
                                  np.interp(nn[1], x, y) + offset + cross_height], '#6b8e23')
        offset = max(y) + offset + 5
