            ...
            for i in x:
                for j in y:
                    if i != 0 and i != 10 and j != 0 and j != 15:
                        #interior value, compute average of neighbor pts, update immediately
                        newV.itemset((i,j),(newV[i-1,j]+newV[i+1,j]+newV[i,j-1]+newV[i,j+1])/4.)
...
############
    ...
    if 1:
        fig = plt.figure()
        plt.suptitle('Gauss-Seidel Method')
